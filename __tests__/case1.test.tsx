...
describe("bookSlice", () => {
  it("bookSlice reducer works correctly", () => {
    const afterAddReducer1 = bookSlice(
      initialState,
      addBooking(testBookingData1)
    );
    expect(afterAddReducer1.bookItems.length).toBe(1);
    const afterAddReducer2 = bookSlice(
      afterAddReducer1,
      addBooking(testBookingData2)
    );
    expect(afterAddReducer2.bookItems.length).toBe(2);
    const afterAddReducer3 = bookSlice(
      afterAddReducer2,
      addBooking(testBookingData3)
    );
    expect(afterAddReducer3.bookItems.length).toBe(2);

    const afterRemoveReducer = bookSlice(
      afterAddReducer3,
      removeBooking(testBookingData2)
    );
